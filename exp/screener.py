...
def update_paper_portfolio(screen_df:pd.DataFrame):
    ...
    current_weights = {position.symbol:float(position.market_value)/portfolio_value for position in current_positions}
    ...
    for symbol in current_weights.keys():
        if symbol not in screen_df['attributes.name'].tolist():
            # now clearing positions that are to be removed from the portfolio
    ...
    symbols = set(screen_df['attributes.name'].tolist() + list(amount_change.keys()))
